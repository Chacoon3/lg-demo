...
def test_entry_router_call_raises_not_implemented():
    ...
    state = RuntimeState(messages=[AIMessage(content="x")], llm_calls=0, tool_calls=0)
...
def test_prompt_class_router_routes_to_matching_classification():
    ...
    state = RuntimeState(messages=[AIMessage(content="math")], llm_calls=0, tool_calls=0)
...
def test_prompt_class_router_returns_end_when_not_in_exit_set():
    ...
    state = RuntimeState(messages=[AIMessage(content="unknown")], llm_calls=0, tool_calls=0)
...
def test_tool_call_router_returns_end_without_tool_calls_or_unknown_tool():
    ...
    no_call_state = RuntimeState(messages=[AIMessage(content="")], llm_calls=0, tool_calls=0)
    ...
    assert router(unknown_call_state) == router.END
